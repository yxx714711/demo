...
public class GithubCrawler extends AbstractCrawler {
    ...
    public FetchResult crawl(CrawlerContext context) {
        ...
        String url = String.format(TRENDING_URL, context.getLanguage(), periodParam);
    ...
    private int resolveTopN(PeriodEnum period) {
        ...
        switch (period) {
            case DAILY:
                return config.getDailyTopN();
            case WEEKLY:
                return config.getWeeklyTopN();
            case MONTHLY:
                return config.getMonthlyTopN();
            default:
                return config.getDailyTopN();
        }
    }
